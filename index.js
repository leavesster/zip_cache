...
// Check if the application is installed by checking the controller. If there is a service worker controlling this page, let’s assume the application is installed.
...
// During installation, once the service worker is active, we shows the image dynamic loader.
...
// Install the worker is no more than registering. It is in charge of downloading the package, decompress and cache the resources.
...
// Uninstalling the worker is simply unregistering it. Notice this wont erase the offline cache so the resources are actually still installed but there is no service worker to serve them.
...
$('#uninstall').onclick = function() {
  ...
  navigator.serviceWorker.getRegistration().then(function(registration) {
    ...
    console.error("uninstall err: ", e);
  })
};

// To load an image is no more than assiging the correct URL to the displayer.
